...
- **Fixed (2026-09-17): circuit breaker could still open new positions after tripping mid-run.** `circuit_breaker.check()` only ran once, at the very top of `run_execution()`, before reconciliation/exit-processing. A forced manual run on 2026-09-17 closed 5 consecutive losers via reconciliation (GPI, VTS, CNC, RSG, T), tripped `consecutive_losses_5` on the 5th close, then proceeded to open 6 brand-new positions anyway because the breaker was never re-checked before the entry loop. Now re-checked immediately after `_process_exits()`, before any new entry — a trip discovered mid-run now blocks that same run's entries, not just the next one.
- **Temporarily disabled (2026-09-17): `circuit_breakers.enabled: false`** in `production.yaml`, at Nish's request, so demo-account testing isn't interrupted by halts while validating the pipeline. The mid-run re-check fix above is still in place underneath and takes effect immediately on flipping this back to `true` — do this before any real-funded account.
- **Added (2026-09-17): optional `min_entry_price` floor** in `risk.yaml` (`global.min_entry_price`, default `0.0` = off, zero behavior change). Added after SABR ($2.42/share) hit MT5 lot-step rounding hard enough to land realized risk 68.4% below the 1% target ($273.80 vs $866.02 actual). Not backtested — treat as a live-only safety valve, not a strategy change, if it ever gets set above 0.
- **Added (2026-09-17): `universe.excluded_tickers`** in `production.yaml` — the 9 tickers confirmed permanently unfetchable via TwelveData (`AAC-U`, `BF-A`, `CRD-A`, `EA`, `HEI-A`, `LPRO`, `MOG-A`, `SKYT`, `UHAL-B`) were being re-attempted and re-erroring every single cache-refresh cycle, spamming the residual-errors Telegram alert with the same names daily. Both `build_td_cache.py` and `orchestrator.py` now filter them out before touching the universe at all.
- **Root-caused and fixed (2026-09-17): scheduler job race condition, not a stale-cache bug.** `tools/check_cache_freshness.py` (added same day) confirmed META/OKTA/ZS/EPAM/OSPN/PD/PLMR/RBLX/RSG/SMMT/T's cached parquet files were NOT stale — all had correct, current bars through 2026-09-16. The frozen `signal_date=2026-09-14` values seen in the 2026-09-16 17:05 ET signal run were instead caused by `scheduler.py`'s `BlockingScheduler` using APScheduler's default multi-threaded executor, which lets different cron jobs run concurrently: that day's cache-refresh job (still catching up after a multi-day gap) was only ~34% through re-fetching the universe (`batch=10/30`) when the signals job fired 20 minutes later per its own cron schedule, so signal generation read a half-updated cache mid-flight -- tickers already re-fetched showed fresh data, tickers not yet reached still showed pre-refresh values. Fixed by pinning the scheduler to a single-worker `ThreadPoolExecutor`, so cache refresh, signals, execution, and heartbeat can never overlap -- a job whose cron time arrives mid-run of another job now waits instead of racing it (within each job's existing 1800s `misfire_grace_time`). No execution risk existed from this either way -- the staleness gate already voids any signal older than one trading day before it can be acted on -- but it was producing wrong-looking signal data and log noise.

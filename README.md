...
- **Added (2026-09-17): `tools/check_cache_freshness.py`**, a read-only diagnostic dumping a ticker's `meta.json` + last 3 cached rows directly. Built to investigate META/OKTA/ZS/EPAM/OSPN/PD/PLMR/RBLX/RSG/SMMT/T all logging `signal_date=2026-09-14` again in the 2026-09-16 17:05 ET signal run, two days after that date should have rolled forward — not yet root-caused; the staleness gate (§18, 2026-09-14 entry) already prevents any execution risk from this regardless of the outcome, so it's a data-hygiene item, not a live-risk one.
